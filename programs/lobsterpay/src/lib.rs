...
pub mod lobsterpay {
    ...
    pub fn ensure_vault_token_account(ctx: Context<EnsureVaultTokenAccount>) -> Result<()> {
        instructions::ensure_vault_token_account::handler(ctx)
    }

    pub fn execute_pay_exact(ctx: Context<ExecutePayExact>, params: ExecutePayExactParams) -> Result<()> {
        instructions::execute_pay_exact::handler(ctx, params)
    }

    pub fn execute_swap_exact_in(ctx: Context<ExecuteSwapExactIn>, params: ExecuteSwapExactInParams) -> Result<()> {
        instructions::execute_swap_exact_in::handler(ctx, params)
    }

    pub fn withdraw_owner(ctx: Context<WithdrawOwner>, params: WithdrawOwnerParams) -> Result<()> {
